...
local function updateGFX(dt) -- ms
    ...
    speedDiff = math.floor(math.max(math.min(((electrics.values.airspeed+1)/electrics.values.wheelspeed),1.0),0.0)*100)/100
    ...
    local speedDiffBetweenFrontAndRear = math.max(math.min(math.floor(wheelSpeedFront/wheelSpeedRear*100)/100 + 0.05, 1), 0.33)
    ...
    obj.debugDrawProxy:drawText2D(float3(40,200,0), color(0,125,0,255), "SpeedDiffAxles: " .. speedDiffBetweenFrontAndRear)
